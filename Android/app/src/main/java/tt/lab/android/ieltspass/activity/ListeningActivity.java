...
public class ListeningActivity extends FragmentActivity {
		...
		if (checkNetwork(fromLocal)) {
			...
			player.setOnPreparedListener(new OnPreparedListener() {
				...
				public void onPrepared(MediaPlayer mp) {
					...
					percentage.setText("");
					// tt.lab.android.ieltspass.Logger.i(TAG, "onPrepared " + 4 + ", " + player.getDuration());
			...
			player.setOnBufferingUpdateListener(new OnBufferingUpdateListener() {
				...
				public void onBufferingUpdate(MediaPlayer mp, int percent) {
					...
					// tt.lab.android.ieltspass.Logger.i(TAG, "onBufferingUpdate: " + percent + "% " + progress + ", " + seekBar.getMax());
			...
			try {
				player.reset();
				// tt.lab.android.ieltspass.Logger.i(TAG, "initPlayer " + 2 + ", " + url);
				percentage.setText("Setting");
				player.setDataSource(url);
				// tt.lab.android.ieltspass.Logger.i(TAG, "initPlayer " + 3);
				percentage.setText("Preparing.");
				player.prepareAsync();
				// tt.lab.android.ieltspass.Logger.i(TAG, "initPlayer " + 4);
				percentage.setText("Preparing..");
				// tt.lab.android.ieltspass.Logger.i(TAG, "initPlayer " + 5);
	...
	private void start() {
		if (player != null) {
			// tt.lab.android.ieltspass.Logger.i(TAG, "start I");
			try {
		...
		// tt.lab.android.ieltspass.Logger.i(TAG, "start O");
	}

	private void pause() {
		// tt.lab.android.ieltspass.Logger.i(TAG, "pause I ");
		try {
			if (player != null) {
		...
		// tt.lab.android.ieltspass.Logger.i(TAG, "pause O");
	}

	private void release() {
		// tt.lab.android.ieltspass.Logger.i(TAG, "release I");
		try {
			if (player != null) {
				// tt.lab.android.ieltspass.Logger.i(TAG, "release: 0: ");
				refreshButtonText();
				// tt.lab.android.ieltspass.Logger.i(TAG, "release: 1: ");
				player.release();
				// tt.lab.android.ieltspass.Logger.i(TAG, "release: 2: "+updateProgressThread);

				// tv.setText("播放发生异常!");
				handler.removeCallbacks(updateProgressThread);
				// tt.lab.android.ieltspass.Logger.i(TAG, "release: 3: ");
			}
		} catch (Exception e) {
			Logger.e(TAG, "release: E: " + e.getMessage());
			e.printStackTrace();
		}
		// tt.lab.android.ieltspass.Logger.i(TAG, "release O");
	}
	...
	private class DownloadAsyncTask extends AsyncTask<Integer, Integer, String> {
		...
		protected String doInBackground(Integer... arg0) {
			...
			try {
				// tt.lab.android.ieltspass.Logger.i(TAG, "doInBackground " + arg0);
				...
				while ((len = is.read(buffer)) != -1) {
					read += len;
					// tt.lab.android.ieltspass.Logger.i(TAG, "doInBackground "+3+", "+len);
					os.write(buffer, 0, len);
				}
				// is.close();
				// os.close();

				// tt.lab.android.ieltspass.Logger.i(TAG, "doInBackground read " + read);
				if (read > 1024 * 1024) {// 1M
					File file = new File(filename);
					boolean renameTo = tmp.renameTo(file);
					// tt.lab.android.ieltspass.Logger.i(TAG, "doInBackground renameTo " + renameTo);
				} else {
					boolean delete = tmp.delete();
					// tt.lab.android.ieltspass.Logger.i(TAG, "doInBackground delete " + delete);
				}
			} catch (Exception e) {
		...
		protected void onPreExecute() {
			downloading = true;
			// tt.lab.android.ieltspass.Logger.i(TAG, "onPreExecute");
		}

		@Override
		protected void onPostExecute(String result) {
			downloading = false;
			// tt.lab.android.ieltspass.Logger.i(TAG, "onPostExecute: " + result);
		}

		@Override
		protected void onProgressUpdate(Integer... values) {
			int value = values[0];
			// tt.lab.android.ieltspass.Logger.i(TAG, "onProgressUpdate: " + value);
