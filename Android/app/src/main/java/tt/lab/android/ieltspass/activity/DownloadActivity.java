						...
						if (currentView instanceof Button) {// Button
							...
							button.setOnClickListener(new OnClickListener() {
								...
								public void onClick(View v) {
									...
									//tt.lab.android.ieltspass.Logger.i(TAG, "preparing to download: "+map+", "+position+", "+start);
									// 控制开始停止
									if (start) {
			...
			try {
				if (state == DOWNLOAD_INCOMPLETE) {
					...
					while ((len = is.read(buffer)) != -1 && isStart(position)) {
						...
						//tt.lab.android.ieltspass.Utilities.writeFile(infoAbsFileName, String.valueOf(progress));// 保证外存中最新
					}
					if(progress!=0){
