...
public class EmailSender {
	...
	public static void sendEmail(String host, String address, String from, String password, String to, String port,
		...
		//tt.lab.android.ieltspass.Logger.i("Check", "done pops");
		Session session = Session.getDefaultInstance(props, null);
		//DataHandler handler = new DataHandler(new ByteArrayDataSource(finalString.getBytes(), "text/plain"));
		MimeMessage message = new MimeMessage(session);
		message.setFrom(new InternetAddress(from));
		//message.setDataHandler(handler);
		//tt.lab.android.ieltspass.Logger.i("Check", "done sessions");

		multiPart = new MimeMultipart();
		InternetAddress toAddress;
		toAddress = new InternetAddress(to);
		message.addRecipient(Message.RecipientType.TO, toAddress);
		//tt.lab.android.ieltspass.Logger.i("Check", "added recipient");
		message.setSubject(subject);
		message.setContent(multiPart);
		message.setText(content);

		//tt.lab.android.ieltspass.Logger.i("check", "transport");
		Transport transport = session.getTransport("smtp");
		//tt.lab.android.ieltspass.Logger.i("check", "connecting");
		transport.connect(host, address, password);
		//tt.lab.android.ieltspass.Logger.i("check", "wana send");
		transport.sendMessage(message, message.getAllRecipients());
		transport.close();
		//tt.lab.android.ieltspass.Logger.i("check", "sent");
	}
}
